...
def main() -> None:
    ...
    # Cập nhật khối đo thời gian Inference
    ...
    logger.info("Thời gian xử lý nhận diện (Inference Time): %.4f giây", inference_time_seconds)

    prediction_dicts = [prediction.to_dict() for prediction in predictions]

    save_dir = IOUtils.ensure_dir(args.save_dir)
    ...
    # Cập nhật Data JSON để bao gồm inference_time_seconds
    IOUtils.save_json(
    ...
    logger.info("Num predictions: %d", len(prediction_dicts))
